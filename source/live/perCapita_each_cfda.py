...
def tally_state_totals(cfda_csv_list, state_list):
	for row in cfda_csv_list:
		if row[-4] == 'USA':  # index in sublist of country column
			# print("state: ", row[-5], " value: ", row[5])
			for state in state_list:
				if state[2] == row[-5]:  # state abbreviation from populations list, and state abbreviation column from cfda csv
					state[4] = append_sum_strs(state[4], row[5])  # because data is stored as strings
	return state_list


def calculate_per_capita_spending(state_list):
	for state in state_list:
		state[6] = divide_strs(state[4], state[1])
	return state_list


def get_per_cap(s):
	return float(s[6])  # 5th column is per cap spending


def get_total_spending(s):
	return float(s[4])  # 4th column is total spending
